...
export default function GoogleLoginButton() {
  ...
  const handleGoogleLogin = async () => {
    setIsLoading(true);
    try {
      const data = await fetchAPI<{ url: string }>('/auth/google/redirect');
      ...
      console.error(error);
      alert("Terjadi kesalahan pada server backend. Pastikan Laravel berjalan normal.");
      setIsLoading(false);
    }
  };
