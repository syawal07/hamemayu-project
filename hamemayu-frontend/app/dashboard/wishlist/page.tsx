...
export default function WishlistPage() {
  ...
  useEffect(() => {
    ...
    const loadWishlists = async () => {
      ...
      try {
        const res = await fetchAPI<{ data?: WishlistItem[] } | WishlistItem[]>('/wishlist', { requireAuth: true });
  ...
  const getItemData = (item: WishlistItem) => {
    ...
    let imageUrl = item.plannable?.image || data.cover_image;
    ...
    return {
      ...
      image: imageUrl,
      type: item.plannable?.type || 'App\\Models\\Content'
    };
  };
  ...
  const navigateToItem = async (item: WishlistItem, e: React.MouseEvent) => {
    ...
    try {
      ...
      const markersRes = await fetchAPI<MapMarker[] | null>('/map-markers');
      const markers: MapMarker[] = Array.isArray(markersRes) ? markersRes : [];
      
      if (markers.length === 0) {
        alert("Gagal memuat data peta atau data kosong.");
        return;
      }
      ...
      if (err instanceof Error) {
        alert(`Gagal membuka peta: ${err.message}`);
      } else {
        alert("Gagal membuka peta: Unknown error");
      }
    }
  };
  ...
  const navigateAll = async () => {
    try {
      ...
      if (destinations.length === 0) { 
        alert("Tidak ada destinasi dengan koordinat."); 
        return; 
      }
    ...
    <div className="animate-in fade-in duration-500 max-w-5xl mx-auto pb-12">
      ...
      {loading ? (
        <div className="flex flex-col gap-5">
          {[1, 2, 3].map(i => <div key={i} className="h-32 bg-white/40 dark:bg-slate-800/40 backdrop-blur-md rounded-3xl animate-pulse border border-white/50 dark:border-slate-700/50 shadow-sm"></div>)}
        </div>
      ) : wishlists.length > 0 ? (
                    ...
                    {isEditing ? (
                      <div className="mb-6 relative group/edit" onClick={(e) => e.stopPropagation()}>
                        <textarea
                          ...
                          autoFocus
                          className="w-full p-3 text-xs bg-slate-50 dark:bg-slate-800/50 border border-green-500 dark:border-green-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500/50 resize-none font-mono text-slate-600 dark:text-slate-400 min-h-20 leading-relaxed shadow-sm"
                      ...
                      <div 
                        ...
                        className="bg-slate-50/50 dark:bg-slate-800/30 p-3 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 hover:border-green-500 dark:hover:border-green-500 transition-colors cursor-text group/note mb-6 min-h-12 flex items-center"
                      >
                        <p className="font-mono text-xs text-slate-600 dark:text-slate-400 leading-relaxed break-all w-full">
